...
int Simulation::getNext(const Particle &p, int previousIndex,
  ...
  for (int i = 0; i < p.links.size(); i++) {
    for (int j = 0; j < current.links.size(); j++) {
      if ((p.links[i] == current.links[j]) && (p.links[i] != previousIndex) &&
...
void Simulation::addCollisionForce() {
  ...
  for (const auto &iter : cells) {
    ...
    tree.addPoint(Point{{p.position.x, p.position.y, p.position.z}}, iter.first,
                  false);
  }
  ...
  for (auto &iter : cells) {
    ...
    for (auto neighbor : tree.searchCapacityLimitedBall(
             Point{{p.position.x, p.position.y, p.position.z}}, radiusSquared,
             parameters.maxNeighbors)) {
